#ifndef VOIP_MOBILE_AILIAO_SESSION_H_H_H
#define VOIP_MOBILE_AILIAO_SESSION_H_H_H
#include "voip.h"


class VoipMobileAILIAOSession : public MediaSession
{
public:
	VoipMobileAILIAOSession(DATEHEADER& packh, int aT, char * Packet);
	virtual ~VoipMobileAILIAOSession();	
	virtual int InsertPacket(DATEHEADER& dataheader,const char *pPacket);
	virtual int ListToFile();
	static BOOL IsVoipMobileAILIAO(DATEHEADER& packh,const char *pPacket);
