...
BOOL VoipMobileHHCALLSession::IsVoipMobileHHCALL(DATEHEADER& dataheader,const char *pPacket)
{
	...
	if ( (dataheader.m_datalen == 45) &&
		 ( (*(unsigned short *)pPacket == 0x0380) || (*(unsigned short *)pPacket == 0X8380) ) )
	{
		return TRUE;
	}
	return FALSE;
...
VoipMobileHHCALLSession::VoipMobileHHCALLSession(DATEHEADER& packh, int aT, char * pPacket) : MediaSession(packh,aT)
{
	m_processtm				= packh.m_tm;
	m_begfiletm				= packh.m_tm;
	m_Packettype			= packh.m_type;
	m_MediaPro				= _T("voip_mobile_hhcall");

	m_VoipFileHdr.Protocol	= 0x50;
...
VoipMobileHHCALLSession::~VoipMobileHHCALLSession(){}



int VoipMobileHHCALLSession::InsertPacket(DATEHEADER &packh, const char * pPacket)
{
...
int VoipMobileHHCALLSession::ListToFile()
{
...
//	m_CalleePackList.sort();
	...
	{
		m_realfname = CreateMediaFilename("voip_mobile_hhcall", m_sip, m_dip);
	...
	{
		WriteLog(MEDIAPRO, "ListToFile() In VOIP MOBILE HHCALL Error!!");
		return -3;
	}
	return 0;
}
