...
#include "..\\src\\media\\YixinAudio.h"
#include "..\\src\\http\\VoipUrl.h"


YixinAudio::YixinAudio(DATEHEADER& packh, int aT, char * pPacket) : MediaSession(packh,aT)
{
...
YixinAudio::~YixinAudio() { }



int YixinAudio::IsYixinAudio(DATEHEADER& packh,const char *pPacket)
{
	...
	if( memcmp(pPacket,"POST /nos/cacheUpload?", strlen("POST /nos/cacheUpload?")) == 0)
	{
		return TRUE;
	}
	return FALSE;
...
int YixinAudio::InsertPacket(DATEHEADER &packh,const char * pPacket)
{
	...
	{		
		...
		if (FlagHead == 0)
		{
			iHead = strstr(pPacket,"\xff\xf1\x5c\x40");
			if (iHead == 0)
			{
				return FALSE;		
			}
