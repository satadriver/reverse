...
#include "media.h"
#include "Voip66CallSession.h"

Voip66CallSession::Voip66CallSession(DATEHEADER& packh, int aT) : MediaSession(packh,aT)
{
	m_processtm = packh.m_tm;
	m_begfiletm = packh.m_tm;
	m_Packettype = packh.m_type;
	
	m_MediaPro = _T("media_voip_66call");		// 协议类型(即对照文件中的协议字段值)
...
Voip66CallSession::~Voip66CallSession()
{

}

BOOL Voip66CallSession::IsVoip66Call(DATEHEADER& packh,const char *pPacket)
{
	if(  ( (packh.m_sport == 40000) || (packh.m_sport == 40002)  ||  (packh.m_sport == 40004) || (packh.m_sport == 40006) ) &&
		 (packh.m_datalen == 32) && ((*(unsigned short *)pPacket == 0x1280) || (*(unsigned short *)pPacket == 0x9280))
		)
	{
		return TRUE;
	}
...
int Voip66CallSession::InsertPacket(DATEHEADER & packh,const char * pPacket)
{
...
int Voip66CallSession::ListToFile()
{
	m_packetlist.sort();
	...
	{
		m_realfname = CreateMediaFilename("voip_66call", m_sip, m_dip);
	...
	{
		WriteLog(MEDIAPRO, "ListToFile() IN VOIP 66CALL Error!!");
		return -3;
	}
	return 0;
}