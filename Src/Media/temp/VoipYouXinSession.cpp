...
VoipYouXinSession::VoipYouXinSession(DATEHEADER& packh, int aT) : MediaSession(packh,aT)
{
	...
	m_MediaPro = _T("media_voip_yx");		// 协议类型(即对照文件中的协议字段值)
	m_MediaDirection = _T("<<==>>");	// 音视频方向(即对照文件中的方向字段值)
//	m_IDCSvrType = _T("voipyouxinmedia");		// IDC服务器类型(英文表示) 为空或NULL表示不需要输出IDCServer
//	m_IDCSvrSubType = _T("VOIPYOUXIN");	// IDC服务器类型(中文表示) 为空或NULL表示不需要输出IDCServer
	m_type = "voip_media";
	m_decode = 0x13;
	m_flag = 0;
	m_packet_idx = 0;
