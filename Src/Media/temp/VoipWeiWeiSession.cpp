...
#include "media.h"
#include "VoipWeiWeiSession.h"

VoipWeiWeiSession::VoipWeiWeiSession(DATEHEADER& packh, int aT) : MediaSession(packh,aT)
{
	m_processtm = packh.m_tm;
	m_begfiletm = packh.m_tm;
	m_Packettype = packh.m_type;
	
	m_MediaPro = _T("media_voip_weiwei");		// 协议类型(即对照文件中的协议字段值)
...
VoipWeiWeiSession::~VoipWeiWeiSession()
{

}

BOOL VoipWeiWeiSession::IsVoipWeiWei(DATEHEADER& packh,const char *pPacket)
{
	
	if(  (packh.m_sport == 7078)  && (packh.m_datalen == 32) && 
		( (*(unsigned short *)pPacket == 0x1280) || 
...
int VoipWeiWeiSession::InsertPacket(DATEHEADER & packh,const char * pPacket)
{
...
int VoipWeiWeiSession::ListToFile()
{
	m_packetlist.sort();
	...
	{
		m_realfname = CreateMediaFilename("voip_weiwei", m_sip, m_dip);
	...
	{
		WriteLog(MEDIAPRO, "ListToFile() IN VOIP WEIWEI Error!!");
		return -3;
	}
	return 0;
}