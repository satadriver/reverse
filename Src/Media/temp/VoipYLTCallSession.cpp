...
#include "media.h"
#include "VoipYLTCallSession.h"

VoipYLTCallSession::VoipYLTCallSession(DATEHEADER& packh, int aT) : MediaSession(packh,aT)
{
	...
	m_MediaPro = _T("media_voip_ylt");		// 协议类型(即对照文件中的协议字段值)
...
VoipYLTCallSession::~VoipYLTCallSession()
{

}


BOOL VoipYLTCallSession::IsVoipYLT(DATEHEADER& packh,const char *pPacket)
{
	if (((*(unsigned short *)pPacket == 0x1280) || (*(unsigned short *)pPacket == 0x9280)) && 
		(32 == packh.m_datalen) && (packh.m_dport == 15004) && (packh.m_sport != 15004))
	{
		return TRUE;
	}
...
int VoipYLTCallSession::InsertPacket(DATEHEADER & packh,const char * pPacket)
{
...
int VoipYLTCallSession::ListToFile()
{
	m_packetlist.sort();
	...
	{
		m_realfname = CreateMediaFilename("voip_ylt", m_sip, m_dip);
	...
	{
		WriteLog(MEDIAPRO, "ListToFile() IN VOIP YLT Error!!");
		return -3;
	}
	return 0;
}