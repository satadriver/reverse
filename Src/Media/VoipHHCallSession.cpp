...
BOOL VoipHHCallSession::IsVoipHHCall(DATEHEADER& packh,const char *pPacket)
{
	...
	if( (packh.m_datalen == 172) && 
		( (*(unsigned short *)pPacket == 0x0080) || (*(unsigned short *)pPacket == 0x8080) ) )			
	{
		return TRUE;
	}
	return FALSE;
...
VoipHHCallSession::VoipHHCallSession(DATEHEADER& packh, int aT, char * pPacket) : MediaSession(packh,aT)
{
	m_processtm = packh.m_tm;
	m_begfiletm = packh.m_tm;
	m_Packettype = packh.m_type;
	m_MediaPro = _T("media_voip_hhcall");	

	m_VoipFileHdr.Protocol = 0x40;
...
VoipHHCallSession::~VoipHHCallSession(){ }


int VoipHHCallSession::InsertPacket(DATEHEADER & packh,const char * pPacket)
{
...
int VoipHHCallSession::ListToFile()
{
...
//	m_CalleePackList.sort();
	...
	{
		m_realfname = CreateMediaFilename("voip_hhcall", m_sip, m_dip);
	...
	{
		WriteLog(MEDIAPRO, "ListToFile() IN VOIP HHCALL Error!!");
		return -3;
	}
	return 0;
}
