...
void SplitStringQQTBZS(CString strSrc ,CStringArray &arrayDest,char sep)
{
	CString tmpstr;
...
CString GetItemValueQQTBZS(const CString &str,char sep)
{
	if(str.Find(sep) != -1)
	{
...
void WriteQQPimIndexFile(DATEHEADER & stPackHeader,HttpProtocol * pHttpPro, char * Data,QQPIMDATA * QQpimdata)
{
	...
	strData.Replace("BEGIN:VCARD","&");
	strData.Replace("END:VCARD","&");
	strData.Replace(";","#");
	strData.Replace("\r\n",";");
	strData.Replace("\r",";");
	strData.Replace("\n",";");
	CStringArray arrTemp1;
	SplitStringQQTBZS(strData,arrTemp1,'&');
	int arrsize1 = arrTemp1.GetSize();
	for (int i = 0; i < arrsize1; i++)
	{
		CStringArray arrTemp2;
		SplitStringQQTBZS(arrTemp1[i],arrTemp2,';');

		CString strName;
		CString strPhone;
		for (int j = 0;j < arrTemp2.GetSize();j++)
		{
			
			if (strName.IsEmpty())
			{
				if (arrTemp2[j].Find("N#") != -1)
				{
					strName = GetItemValueQQTBZS(arrTemp2[j],':');
					strName.Replace("#","");
					strName.Replace("CHARSET=UTF-8","");

				}
			}
		
			if (arrTemp2[j].Find("FN#") != -1)
			{
				strName = GetItemValueQQTBZS(arrTemp2[j],':');
				strName.Replace("#","");
				strName.Replace("CHARSET=UTF-8","");
			...
			if (arrTemp2[j].Find("CELL:") != -1)
			{
				strPhone = GetItemValueQQTBZS(arrTemp2[j],':');
				strPhone.Replace(" ","");
				strPhone.Replace("+86","");
			}
		}
		...
		if (!strName.IsEmpty() && !strPhone.IsEmpty())
		{
			...
			Txt.Format("user=%s\r\nimei=%s\r\nimsi=%s\r\nnick=%s\r\ncontact=%s\r\n",QQpimdata->QQ,QQpimdata->Imei,QQpimdata->Imsi,strName,strPhone);
