...
{
	memset(QQPimData->QQ,0,QQPIMDATA_ITEM_LEN);
	QQPimData->QQLen		= 0;
	memset(QQPimData->Imei,0,QQPIMDATA_ITEM_LEN);
	QQPimData->ImeiLen		= 0;
	memset(QQPimData->Imsi,0,QQPIMDATA_ITEM_LEN);
	QQPimData->ImsiLen		= 0;
	memset(QQPimData->QQPimKey,0,QQPIMDATA_ITEM_LEN);	
...
QQSyncAsist::QQSyncAsist()
{
	...
	memset(this->DataBuf,0,QQSYNCASIST_DATABUF_LEN);
	memset(this->UpDataBuf,0,QQSYNCASIST_UPDATABUF_LEN);
	this->DataLen		= 0;
	this->ContentLen	= 0;
	this->UpDataLen		= 0;
	this->Time			= time(0);
...
QQSyncAsist::QQSyncAsist(int Flag)
{
	InitQQPimData(&(this->QQPimData));
	memset(this->DataBuf,0,QQSYNCASIST_DATABUF_LEN);
	memset(this->UpDataBuf,0,QQSYNCASIST_UPDATABUF_LEN);
...
int ProcVMessage(char * Buf, int BufLen, char * DestBuf, int * DestLen, char * HeaderFlag, char * EnderFlag, int Offset)
{
...
int GetIPPort(unsigned char * IPPort, unsigned char * IP, unsigned short * Port)
{
	...
	int Number = 0;
	unsigned char * Header = IPPort;
	unsigned char * Ender = 0;
	
	memset(IP,0,4);
	* Port = 0;
	for (int Cnt = 0; Cnt <3; Cnt ++)
	{
		Ender = (unsigned char*)strchr((char*)Header,'.');
		if (Ender == 0)
		{
			return FALSE;
		}
		...
		memset(strIP,0,4);
		memmove(strIP, Header, Ender - Header);
		IP[Number] = atoi((char*)strIP);
		Number ++;
		Header = Ender + 1;
	}
	
	Ender = (unsigned char*)strchr((char*)Header,'_');
	if (Ender == 0)
	{
		return FALSE;
	}
...
//如果没有IMSI则不打印输出
void WriteMyQQPimIndexFile(DATEHEADER & stPackHeader,HttpProtocol * pHttpPro, CString UserName, CString UserPhone,QQPIMDATA * QQpimdata)
{
...
void WriteMyQQPimMsgIndexFile(DATEHEADER & stPackHeader,HttpProtocol * pHttpPro, CString UserPhone,CString UserName, CString UserMsg,
							  QQPIMDATA * QQpimdata)
{
	HttpSession httpsession(stPackHeader, g_httpFrom);		
	httpsession.m_dceprocotol=pHttpPro;
	...
	CString tmppath = httpsession.m_dceprocotol->m_httpMissObj->GetTmpPath();
	ASSERT(!tmppath.IsEmpty());
	httpsession.m_indexfname.Format("%s%04d%02d%02d%02d%02d%02d_%s_%s_1_%u_SMS_QQTBZS.indexfile.txt", 
		tmppath, stNow.wYear, stNow.wMonth, stNow.wDay, stNow.wHour,stNow.wMinute, stNow.wSecond, httpsession.m_sip, httpsession.m_dip, rand());
	CString Txt;
	Txt.Format("user=%s\r\nimei=%s\r\nimsi=%s\r\nnick=%s\r\ncontacts=%s\r\nmsg=%s\r\n",
		QQpimdata->QQ,QQpimdata->Imei,QQpimdata->Imsi,UserName,UserPhone,UserMsg);
//	httpsession.WriteIndexFile("SMS_QQTBZS","SMS_QQTBZS",Txt,httpsession.m_dceprocotol->m_httpMissObj);	
	return;
}
...
void MySplitString(CString strSrc ,CStringArray &arrayDest,char * sep, int seplen)		//VCARD是标准字符串 可以放心处理
{
	CString tmpstr;
...
CString MyGetItemValue(const CString &str,char * sep)
{
...
int ProcMyVObject(char * Buf, int BufLen,char * HeaderFlag, char * EnderFlag, QQPIMDATA * QQpimdata,DATEHEADER &packh,HttpProtocol * HttpPro)
{
	if( BufLen < 32 )
	{
		return FALSE;
...
int ProcMyVMessage(char * Buf, int BufLen,char * HeaderFlag, char * EnderFlag, QQPIMDATA * QQpimdata,DATEHEADER &packh,HttpProtocol * HttpPro)
{
	if( BufLen < 32 )
	{
		return FALSE;
	...
	while ( Header - Buf < BufLen)
	{
		...
		WriteMyQQPimMsgIndexFile(packh,HttpPro,UserPhone,UserName,UserMsg,QQpimdata);
