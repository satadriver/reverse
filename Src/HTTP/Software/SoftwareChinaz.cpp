...
software::Chinaz::Chinaz(HttpSession *http_session, const string &pro_name):Software(http_session,pro_name)
{
	app_ = "站长下载";	
}

software::Chinaz::~Chinaz()
{
	
}

int software::Chinaz::Is(HttpSession *http_session)
{
	//搜索
	if((http_session->m_Requestline.m_Host.Find("down.chinaz.com")!=-1) && 
		(http_session->m_Requestline.m_URI.Find("/query.asp")!=-1)) 
	{
		return kWebSearch;
	}
	//下载
...
int software::Chinaz::Process(const char *packet,int action)
{
	//搜索
...
//搜索
int software::Chinaz::WebSearch(const char *packet)
{
...
//下载
int software::Chinaz::WebDown(const char *packet)
{
   string url = http_session_->m_Requestline.m_URI.GetBuffer(0);
